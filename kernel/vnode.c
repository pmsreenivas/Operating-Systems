...
{       
    ...
    dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
       ...
       // dbg(DBG_PRINT, "(GRADING2S VN_2)\n");
       //      return -ENOTSUP;
       //  }
       dbg(DBG_PRINT, "(GRADING2B)\n");
        return file->vn_cdev->cd_ops->read(bytedev_lookup(file->vn_devid),offset,buf,count);
}
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
    ...
    dbg(DBG_PRINT, "(GRADING2A 1.b)\n");
       ...
       //  if(S_ISBLK(file->vn_mode)){
       // dbg(DBG_PRINT, "(GRADING2S VN_5)\n");
       //      return -ENOTSUP;
       //  }
       dbg(DBG_PRINT, "(GRADING2B)\n");
        return file->vn_cdev->cd_ops->write(bytedev_lookup(file->vn_devid),offset,buf,count);
