...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
		...
		memset(thread, 0, sizeof(kthread_t));
                ...
                context_setup(&thread->kt_ctx, func, (int)arg1, arg2, (void*)thread->kt_kstack, DEFAULT_STACK_SIZE, p->p_pagedir);
		thread->kt_retval = (void *)0;
                thread->kt_errno = 0;
                thread->kt_proc = p;
		thread->kt_wchan = NULL;
		//sched_queue_init(thread->kt_wchan);
                thread->kt_cancelled = 0;
		thread->kt_state = KT_NO_STATE;
                list_insert_tail(&p->p_threads, &thread->kt_plink);
		if (thread->kt_proc->p_pid == PID_IDLE) {
			curthr = thread;
